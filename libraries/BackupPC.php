...
namespace clearos\apps\backuppc;
...
class BackupPC extends Daemon
{
    ...
    public function is_password_set($username)
    {
        ...
        try {
	     $file = new File(self::FILE_USER_CONFIG);
            $retval = $file->lookup_value("/^backuppc/i");
            if ($retval == 0)
                return FALSE;
            else
                return TRUE;
    ...
    public function is_root_password_set()
    {
        ...
        if ($this->is_password_set('backuppc'))
            return TRUE;
        else
            return FALSE;
    }
    ...
    public function validate_password_verify($password, $verify)
    {
        ...
        if ($password != $verify)
            return lang('backuppc_password_mismatch');
    }
    ...
    public function validate_username($username)
    {
        ...
        if (! preg_match('/^([a-z0-9_\-\.\$]+)$/', $username))
            return lang('backuppc_username_invalid');
    }
}
